...
const successResponse = (res, data, count, req, extra = {}) => {
  ...
  res.json({
    ...
    data,
    meta: { requestId: req.requestId, responseTime, ...extra }
  });
};
...
const authenticateApiKey = async (req, res, next) => {
  ...
  try {
    ...
    if (result.rows.length === 0) return errorResponse(res, 401, 'INVALID_API_KEY', 'Invalid API key');
    const user = result.rows[0];
    const plan = user.plan || 'free';
    const limits = PLAN_LIMITS[plan] || PLAN_LIMITS.free;
    res.setHeader('X-RateLimit-Limit', limits.daily);
    res.setHeader('X-RateLimit-Remaining', limits.daily);
...
app.post('/auth/register', async (req, res) => {
  try {
    ...
    if (!name || !email || !password) return errorResponse(res, 400, 'INVALID_QUERY', 'Name, email and password required');
    const existing = await pool.query('SELECT id FROM users WHERE email = $1', [email]);
    if (existing.rows.length > 0) return errorResponse(res, 400, 'INVALID_QUERY', 'Email already registered');
    ...
    successResponse(res, { ...result.rows[0], api_secret: apiSecret, warning: 'Save your API secret! It will not be shown again.' }, undefined, req);
  } catch (err) {
    errorResponse(res, 500, 'INTERNAL_ERROR', err.message);
  }
});
...
app.post('/auth/login', async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) return errorResponse(res, 401, 'INVALID_API_KEY', 'Invalid credentials');
    const user = result.rows[0];
    const valid = await bcrypt.compare(password, user.password);
    if (!valid) return errorResponse(res, 401, 'INVALID_API_KEY', 'Invalid credentials');
    const token = jwt.sign({ id: user.id, email: user.email, role: user.role, plan: user.plan }, JWT_SECRET, { expiresIn: '24h' });
    successResponse(res, { token, user: { id: user.id, name: user.name, email: user.email, plan: user.plan, role: user.role, api_key: user.api_key } }, undefined, req);
  } catch (err) {
    errorResponse(res, 500, 'INTERNAL_ERROR', err.message);
  }
});
...
app.get('/v1/states', authenticateApiKey, async (req, res) => {
  try {
    ...
    successResponse(res, result.rows, result.rows.length, req, {
      rateLimit: { limit: req.limits.daily, remaining: req.limits.daily, reset: new Date(Date.now() + 86400000).toISOString() }
    });
  } catch (err) {
    errorResponse(res, 500, 'INTERNAL_ERROR', err.message);
  }
...
app.get('/v1/states/:id/districts', authenticateApiKey, async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM districts WHERE state_id = $1 ORDER BY name ASC', [req.params.id]);
    if (result.rows.length === 0) return errorResponse(res, 404, 'NOT_FOUND', 'State not found or has no districts');
    successResponse(res, result.rows, result.rows.length, req);
  } catch (err) {
    errorResponse(res, 500, 'INTERNAL_ERROR', err.message);
  }
...
app.get('/v1/districts/:id/subdistricts', authenticateApiKey, async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM sub_districts WHERE district_id = $1 ORDER BY name ASC', [req.params.id]);
    if (result.rows.length === 0) return errorResponse(res, 404, 'NOT_FOUND', 'District not found or has no sub-districts');
    successResponse(res, result.rows, result.rows.length, req);
  } catch (err) {
    errorResponse(res, 500, 'INTERNAL_ERROR', err.message);
  }
...
app.get('/v1/subdistricts/:id/villages', authenticateApiKey, async (req, res) => {
  try {
    ...
    const countResult = await pool.query('SELECT COUNT(*) FROM villages WHERE sub_district_id = $1', [req.params.id]);
    const total = parseInt(countResult.rows[0].count);
    const result = await pool.query('SELECT * FROM villages WHERE sub_district_id = $1 ORDER BY name ASC LIMIT $2 OFFSET $3', [req.params.id, limit, offset]);
    successResponse(res, result.rows, result.rows.length, req, { pagination: { page, limit, total, totalPages: Math.ceil(total / limit) } });
  } catch (err) {
    errorResponse(res, 500, 'INTERNAL_ERROR', err.message);
  }
});
...
app.get('/v1/search', authenticateApiKey, async (req, res) => {
  try {
    ...
    if (!q || q.length < 2) return errorResponse(res, 400, 'INVALID_QUERY', 'Search query must be at least 2 characters');
    let query = `SELECT v.id, v.name as village, sd.name as sub_district, d.name as district, s.name as state,
      'India' as country, CONCAT(v.name, ', ', sd.name, ', ', d.name, ', ', s.name, ', India') as full_address,
      v.id as value, v.name as label
      FROM villages v
      JOIN sub_districts sd ON v.sub_district_id = sd.id
      JOIN districts d ON sd.district_id = d.id
      JOIN states s ON d.state_id = s.id WHERE v.name ILIKE $1`;
    const params = [`%${q}%`];
    let paramCount = 1;
    if (state) { paramCount++; query += ` AND s.name ILIKE $${paramCount}`; params.push(`%${state}%`); }
    if (district) { paramCount++; query += ` AND d.name ILIKE $${paramCount}`; params.push(`%${district}%`); }
    if (subDistrict) { paramCount++; query += ` AND sd.name ILIKE $${paramCount}`; params.push(`%${subDistrict}%`); }
    paramCount++;
...
app.get('/v1/autocomplete', authenticateApiKey, async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      SELECT v.id as value, v.name as label,
        CONCAT(v.name, ', ', sd.name, ', ', d.name, ', ', s.name, ', India') as full_address,
        json_build_object('village', v.name, 'subDistrict', sd.name, 'district', d.name, 'state', s.name, 'country', 'India') as hierarchy
      FROM villages v
      JOIN sub_districts sd ON v.sub_district_id = sd.id
      JOIN districts d ON sd.district_id = d.id
      JOIN states s ON d.state_id = s.id
      WHERE v.name ILIKE $1 ORDER BY v.name ASC LIMIT $2
    `, [`${q}%`, parseInt(limit)]);
    successResponse(res, result.rows, result.rows.length, req);
  } catch (err) {
    errorResponse(res, 500, 'INTERNAL_ERROR', err.message);
...
app.get('/admin/users', authenticateJWT, isAdmin, async (req, res) => {
  try {
    ...
    const offset = (page - 1) * limit;
    let query = `SELECT id, name, email, business_name, plan, role, status, api_key, created_at FROM users WHERE 1=1`;
    const params = [];
    let paramCount = 0;
    if (search) { paramCount++; query += ` AND (email ILIKE $${paramCount} OR name ILIKE $${paramCount})`; params.push(`%${search}%`); }
    if (plan) { paramCount++; query += ` AND plan = $${paramCount}`; params.push(plan); }
...
app.put('/admin/users/:id/plan', authenticateJWT, isAdmin, async (req, res) => {
  try {
    const { plan } = req.body;
    if (!PLAN_LIMITS[plan]) return errorResponse(res, 400, 'INVALID_QUERY', 'Invalid plan');
    const result = await pool.query('UPDATE users SET plan = $1 WHERE id = $2 RETURNING id, name, email, plan', [plan, req.params.id]);
    successResponse(res, result.rows[0], undefined, req);
  } catch (err) {
    errorResponse(res, 500, 'INTERNAL_ERROR', err.message);
  }
...
app.put('/admin/users/:id/status', authenticateJWT, isAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query('UPDATE users SET status = $1 WHERE id = $2 RETURNING id, name, email, status', [status, req.params.id]);
    successResponse(res, result.rows[0], undefined, req);
  } catch (err) {
    errorResponse(res, 500, 'INTERNAL_ERROR', err.message);
  }
...
app.get('/', (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      geo: { states: 'GET /v1/states', districts: 'GET /v1/states/:id/districts', subDistricts: 'GET /v1/districts/:id/subdistricts', villages: 'GET /v1/subdistricts/:id/villages', search: 'GET /v1/search?q=', autocomplete: 'GET /v1/autocomplete?q=' },
